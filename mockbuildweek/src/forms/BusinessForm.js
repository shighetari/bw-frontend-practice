...
const BusinessForm = () => {
    ...
    const handleRegister = data => {
        if(data.password === data.password_verify) {
            ...
            axios.post('https://food-bw-practice-backend.herokuapp.com/api/auth/bussiness-owner/register', data)
                .then(res => {
    ...
    const handleLogin = data => {
        axios.post('https://food-bw-practice-backend.herokuapp.com/api/auth/bussiness-owner/login', data)
        .then(res => {
            console.log(res.data)
            reset()
        })
